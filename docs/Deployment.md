...
Cross-platform preview builds use `scripts/build-cross-platform.ps1`. It compiles the schema-v3 catalogs, builds `cowebs` for Windows x64 and Linux x64/arm64 from one version, creates versioned archives, injects exact archive hashes into the Unix bootstrap, writes `release-manifest-v1.json`, `SHA256SUMS`, an SPDX 2.3 SBOM, and generated Winget manifests. Linux-native packages are built with `scripts/build-linux-packages.sh VERSION ARCH BUNDLE [all|deb|rpm]`; selecting one format keeps Ubuntu and Fedora builders independent. Disposable Ubuntu 24.04 and Fedora 44 builds have installed and verified the generated DEB and RPM respectively, but both remain unsigned preview artifacts.

Do not publish preview output until disposable Ubuntu and Fedora real-install evidence, native-package parity, signing, public-asset re-download verification, and the release-candidate support matrix pass. Publication, repository submission, tag creation, signing, or Windows runtime cutover are separate explicitly authorized actions.
