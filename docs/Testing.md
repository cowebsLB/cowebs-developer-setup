...
- deterministic v2-to-v3 compilation with exact preservation of all 86 packages, 34 packs, nine profiles, Windows mappings, dependencies, conflicts, configuration intents, options, and estimates plus 58 reviewed Ubuntu classifications, 36 executable providers, and seven signed-repository prerequisites;
...
- the full 11-package Ubuntu x64 core schema-v2 compile and deterministic CLI plan, bounded runtime, productivity/tooling, and eleven-provider Kubernetes/IaC/security plans, ordered dependency-aware unsupported diagnostics, exact typed repository digests, shared HashiCorp prerequisites, one de-duplicated APT refresh, Flathub user scope, reviewed classic Snap options, ARM64 unsupported diagnostics, and representative planner-to-adapter detection/prerequisite dry-run rendering without installing software;
