...
- whitespace-safe Windows conflict-diagnostic capture when Windows PowerShell wraps native error output at narrow host widths;
...
Corrected Fedora VM run `31852156303` passed in 4 minutes 29 seconds: VM/RPM provisioning, root-owned installed files, unprivileged doctor and real setup, DNF and Snap completion on the Fedora guest kernel, byte-equal canonical plan output, initiating-user journal ownership, completed-state resume, a second idempotent installation, and unconditional VM deletion. Together with Ubuntu run `31851004282`, the guarded core fresh/resume/idempotency story is complete on both Linux targets. Expanded Ubuntu run `31856228013` and Fedora VM run `31856229843` later passed partial inventory, skips, interruption/resume, forced native-source failure/recovery, PATH/session, ownership, redaction, cleanup, and repeat-run assertions.
...
Final v6.3.0-rc.1 evidence: local `tests/run-tests.ps1` passed in 242.8 seconds before tagging and 226.4 seconds after the publication documentation update; normal CI `31856222715` passed Windows, Ubuntu, and Fedora; Ubuntu disposable matrix `31856228013` passed; Fedora 44 VM matrix `31856229843` passed; release workflow `31856640404` passed build, signature, provenance, draft, public-download, bootstrap, version, and catalog-discovery verification; and post-publication documentation CI `31857144843` passed. An independent unauthenticated audit downloaded all eight manifest-declared artifacts and their signature bundles, reproduced every declared size and SHA-256 digest, and verified GitHub provenance for the manifest and DEB.

The subsequent repository-wide documentation consistency audit passed the focused Windows manifest/profile/bootstrap/documentation test and the complete `tests/run-tests.ps1` suite in 282.6 seconds. The first full attempt exposed a host-width-dependent test assertion: Windows PowerShell split `Package conflict` across error records. The engine still exited correctly with the full diagnostic; changing the assertion to accept whitespace wrapping made the capture portable, and both focused and full reruns passed.
