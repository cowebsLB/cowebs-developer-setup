...
- deterministic v2-to-v3 compilation with exact preservation of all 86 packages, 34 packs, nine profiles, Windows mappings, dependencies, conflicts, configuration intents, options, and estimates plus 40 reviewed Ubuntu classifications, 25 executable providers, and five signed-repository prerequisites;
...
- the full 11-package Ubuntu x64 core schema-v2 compile and deterministic CLI plan, bounded five-provider runtime and nine-provider productivity/tooling plans, ordered unsupported diagnostics, exact typed repository digests, five repository operations with one de-duplicated APT refresh, Flathub user scope, ARM64 unsupported diagnostics, and representative planner-to-adapter detection/prerequisite dry-run rendering without installing software;
