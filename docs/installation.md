...
macOS remains an architectural placeholder. Ubuntu and Fedora have a source-only typed Go adapter foundation for APT, DNF, Snap, and Flatpak. Ubuntu now has 40 reviewed classifications: 25 executable providers and 15 explicit unsupported results across the core, runtime, database/client, browser/networking, design, and Android slices. GitHub CLI, PostgreSQL, Google Chrome, Cloudflare Tunnel, and ngrok use typed, digest-pinned official APT-repository prerequisites; Bruno is explicitly user-scoped to Flathub, and Snap-based providers remain conditional on snapd and store availability. Real repository mutation, downloaded-artifact execution, user-scoped package-manager configuration, the remaining provider catalog, Linux orchestration, native packaging, and the Unix bootstrap are not complete, so there is no supported Linux installation command yet. The released and tested public execution path remains Windows-only.
