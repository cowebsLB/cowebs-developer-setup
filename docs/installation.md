...
macOS remains an architectural placeholder. Ubuntu and Fedora have a source-only typed Go adapter foundation for APT, DNF, Snap, and Flatpak. Ubuntu now has 58 reviewed classifications: 36 executable providers and 22 explicit unsupported results across the core, runtime, productivity/tooling, and Kubernetes/IaC/security slices. Seven official APT repositories use typed, digest-pinned prerequisites; Terraform, Vault, and Packer share one HashiCorp prerequisite. Bruno is explicitly user-scoped to Flathub, and Snap-based providers remain conditional on snapd, store availability, and reviewed confinement. Real repository mutation, downloaded-artifact execution, user-scoped package-manager configuration, the remaining provider catalog, Linux orchestration, native packaging, and the Unix bootstrap are not complete, so there is no supported Linux installation command yet. The released and tested public execution path remains Windows-only.
