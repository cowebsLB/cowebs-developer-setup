...
The Linux adapter follows the same no-shell boundary: catalog fields become individual process arguments, and APT, DNF, Snap, and system Flatpak operations require explicit `elevated`/`machine` metadata. Per-user Flatpak operations require `user`/`user`, Flatpak requires an explicit remote, and distribution-mismatched managers, custom unimplemented sources, option-shaped positional identifiers, malformed installer options, and control characters fail before execution. The compatibility compiler allowlists every Ubuntu mapping field, requires typed option and architecture arrays, and never emits an executable provider for an `unsupported` classification. GitHub CLI therefore remains unsupported instead of embedding repository-setup shell commands. The adapter does not elevate itself; a future controller must separate user operations from one explicit elevated broker handoff.
