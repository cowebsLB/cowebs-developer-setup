...
The shadow planner has exact black-box parity with the production PowerShell planner. The Windows and Linux provider adapters, Windows privileged broker, journal, resume/status flow, and diagnostic CLI have unit and CLI integration coverage, including the principal tamper and recovery boundaries; this is not a claim of exhaustive security proof or real-install validation. Ubuntu tests compile 26 reviewed classifications, prove the complete 11-package x64 core and a bounded five-provider language/runtime plan, verify deterministic JSON and typed GitHub repository ordering, assert complete unsupported-runtime diagnostics, and route representative plan operations through injected detection and dry-run execution. Real prerequisite execution deliberately returns an error. Schema v2 remains authoritative for `src/windows/setup.ps1` and the public release; neither the Go binary nor schema-v3 catalogs are included in that runtime ZIP.
